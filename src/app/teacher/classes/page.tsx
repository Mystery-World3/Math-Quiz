      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        <header className="h-16 bg-card border-b flex items-center justify-between px-4 md:px-8">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-lg md:text-xl font-bold text-primary truncate">Kelola Jenjang Kelas</h1>
          </div>
            ...
            <Dialog open={isModalOpen} onOpenChange={(open) => {
              ...
              <DialogTrigger asChild>
                <Button className="font-bold gap-2 h-9 px-3 sm:h-10 sm:px-4">
                  <Plus className="h-4 w-4" /> <span className="hidden sm:inline">Tambah Kelas</span>
                </Button>
              </DialogTrigger>
              <DialogContent>
                <DialogHeader>
        ...
        <div className="flex-1 overflow-auto p-4 md:p-8">
          <div className="max-w-4xl mx-auto space-y-6">
            <Card>
              <CardHeader>
                <CardTitle className="text-base md:text-lg">Daftar Jenjang Kelas</CardTitle>
              </CardHeader>
                      ...
                      <div key={cls.id} className={`flex items-center justify-between p-4 bg-card rounded-xl border transition-all ${!cls.isActive ? 'opacity-60 bg-muted/50' : 'hover:shadow-sm'}`}>
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex flex-col">
                            <span className="font-bold text-base md:text-lg">{cls.name}</span>
                        ...
                        <div className="flex items-center gap-1 md:gap-2">
                          ...
                          />
                          <Button variant="ghost" size="icon" className="text-blue-500 h-8 w-8" onClick={() => handleEdit(cls)}>
                            <Edit2 className="h-4 w-4" />
                          </Button>
                          <Button variant="ghost" size="icon" className="text-red-500 h-8 w-8" onClick={() => handleDelete(cls.id)}>
